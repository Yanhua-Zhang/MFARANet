...
from engine.engine_test import test_PSP, cal_acc, direct_cal_acc_PSP
...
def main_inference(cfg):
    ...
    data_test = dataset_loader_general.SegDataset(data_root=cfg['data_root'],
                    list_root=cfg['test_list_root'],split='val',max_num=cfg['max_num'],
    ...
    # =============================================================
    # 保存可视化结果
    save_path = cfg['absolute_path']+ cfg['FILE_NAME'] +'/save/image/'+ cfg['NAME_model'] +'_'+ cfg['NAME_dataset'] 
    ...
    # 仅计算 mIoU、acc 等指标
    if cfg['if_direct_get_acc']:
        logger.info('------------>direct_get_acc')
        direct_cal_acc_PSP(test_loader, model, cfg['num_classes'], mean, std, cfg['base_size'], cfg['test_h'], cfg['test_w'], cfg['scales'], logger)

    # 得到 id img,并保存
    if cfg['if_save_image_result']:
        logger.info('------------>save_image_result')
        colors = np.loadtxt(colors_path).astype('uint8')
        test_PSP(test_loader, data_test.img_label_root, model, cfg['num_classes'], 
                mean, std, cfg['base_size'], cfg['test_h'], cfg['test_w'], cfg['scales'], gray_folder, color_folder, colors, logger)
    ...
    if cfg['if_get_acc_from_image_result']:
        ...
        cal_acc(data_test.img_label_root, gray_folder, cfg['num_classes'], names, logger)
