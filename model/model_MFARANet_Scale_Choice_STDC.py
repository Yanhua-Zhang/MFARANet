...
from .backbone.stdcnet import STDCNet813, STDCNet1446   # 加载预训练模型与参数
...
class MFARANet_Scale_Choice_STDC(nn.Module):
    def __init__(self, backbone_name='STDC_V1', use_aux_loss=True, 
                    use_dilation=False, use_PPM=False, ppm_bins=(1, 2, 3, 6), 
                    if_stage1_4_repeat_fuse=False, 
                    ASFM_stage_choose =[1,2,3,4],
                    Dropout_Rate_CNN = [0.0, 0.05, 0.05, 0.05, 0.05],
        ...
        super(MFARANet_Scale_Choice_STDC, self).__init__()
        ...
        # -------------------------------------------------------------------
        # backbone 加载
        if backbone_name == 'STDC_V1':
            backbone = STDCNet813(norm_layer = nn.BatchNorm2d)
            stage_channels = [64, 256, 512, 1024]  # 'resnet18' 各 stage 的输出 channel
        elif backbone_name == 'STDC_V2':
            backbone = STDCNet1446(norm_layer = nn.BatchNorm2d)
            stage_channels = [64, 256, 512, 1024]  # 'resnet18' 各 stage 的输出 channel
        
        # -------------------------------------------------------------------
        # 利用 imagenet 预训练层构建 backbone

        self.layer0 = nn.Sequential(backbone.x2, nn.Dropout2d(self.Dropout_Rate_CNN[0]) if self.Dropout_Rate_CNN[0] > 0. else nn.Identity())

        self.layer1, self.layer2, self.layer3, self.layer4 = nn.Sequential(backbone.x4, nn.Dropout2d(self.Dropout_Rate_CNN[1]) if self.Dropout_Rate_CNN[1] > 0. else nn.Identity()), nn.Sequential(backbone.x8, nn.Dropout2d(self.Dropout_Rate_CNN[2]) if self.Dropout_Rate_CNN[2] > 0. else nn.Identity()), nn.Sequential(backbone.x16, nn.Dropout2d(self.Dropout_Rate_CNN[3]) if self.Dropout_Rate_CNN[3] > 0. else nn.Identity()), nn.Sequential(backbone.x32, nn.Dropout2d(self.Dropout_Rate_CNN[4]) if self.Dropout_Rate_CNN[4] > 0. else nn.Identity())
